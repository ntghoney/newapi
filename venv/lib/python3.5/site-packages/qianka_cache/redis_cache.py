...
class RedisCache(BaseCache):
    ...
    def __init__(self, hosts, marshal_module,
        ...
        self.ring = HashRing(nodes, weights, use_name=hashring_use_name)
        self.clients = {}
    ...
    def add(self, key, value, timeout=None):
        ...
        return (
            ...
            self._set_timeout(client, k, timeout)
        )
    ...
    def dec(self, key, delta=1, timeout=None):
        ...
        k = self._get_key(key)
        client = self._key_to_conn(k)
        rv = client.decr(k, delta)
        self._set_timeout(client, k, timeout)
        return rv

    def _set_timeout(self, client, k, timeout):
        """
        通用的设置过期时间，如果为-1，即特殊的情况，则不设置
        """
        if timeout == -1:
            return 1
        return client.expire(k, timeout)
